...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--movies',
            type=int,
            help=f'This flag will query through DB and write out info about movies starring selected actor in lead role. To select an actor write a positive number after the flag ',
        )
        parser.add_argument(
    ...
    def handle(self, *args, **kwargs):
        if kwargs['add']:
            ...
            if kwargs['movies'] < 1 or kwargs['movies'] > Actors.objects.count():
                ...
                if movies.count() == 0:
                    self.stdout.write(f'There are no movies matching selected actor in the Database :(')
                else:
