...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        if kwargs['add']:
            title = kwargs['title']
            premiere_date = (
                datetime.strptime(kwargs['premiere_date'], '%Y-%m-%d').date() if kwargs['premiere_date'] else None
            )
            ...
            for i, movie in enumerate(movies):
                self.stdout.write(
                    f'{i+1}. "{movie.title}" ({movie.category}) premiered {movie.premiere_date}. It was directed by {movie.director} and starred {movie.lead_actor}. It won {movie.academy_awards} academy awards.'
                )
        elif kwargs['delete']:
