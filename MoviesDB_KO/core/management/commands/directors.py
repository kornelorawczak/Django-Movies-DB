...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--movies', type=int, help = f'This flag will query through DB and write out info about movies directed by selected director. To select a director write a positive number after the flag ')
        parser.add_argument('--name', type=str, help='Full name of a director, prefferably using Capital letters and good name format')
    ...
    def handle(self, *args, **kwargs):
        if kwargs['add']:
            ...
            for i, director in enumerate(directors):
                self.stdout.write(f"{i+1}. {director.name} born on {director.date_of_birth}. The latest movie he/she directed is called {director.latest_movie}")
        elif kwargs['delete']:
            ...
            if kwargs['movies']<1 or kwargs['movies'] > Directors.objects.count():
                ...
                if movies.count()==0: self.stdout.write(f"There are no movies matching selected director in the Database :(")
                else:
                    self.stdout.write(f"Movies directed by {director_selected.name} in the DataBase:")
                    for i, movie in enumerate(movies):
                        self.stdout.write(f'{i+1}. "{movie.title}" released on {movie.premiere_date} starring {movie.lead_actor} in lead role. It has won {movie.academy_awards} Academy Awards.')
